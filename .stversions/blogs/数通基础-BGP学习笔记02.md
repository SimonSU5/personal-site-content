...
1. 应用层报文：BGP公共报文头+BGP packet
	...
	2. packet内容
		1. open
			...
			4. bgp ID（32bit）router id，以IP的形式表示（缺省为peer IP）
			5. opt param len
				1. optional param
		2. update——发布或者撤销路由
...
3. 设计路由反射器时需要考虑防环
	1. cluster-list 路由反射器簇列表
		...
		3. 不能用originator-id防环的情况：
		![[Pasted image 20260725225946.png]]
...
当前8条都一样，无法区分出最优路径，则自动进行负载分担。可以手动关闭来进入9 10 11条。
![[Pasted image 20260725230120.png]]
